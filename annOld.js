(function () {
	...
	function multMatrixScalar (m1, scalar) {
	...
	function subtractMatrixMatrix (m1, m2) {
		for (var i = 0, rows = m1.length; i < rows; i++) {
			for(var j = 0, cols = m1[i].length; j < cols; j++) {
	...
	function backProp (ann, x, y) {
		...
		for(var l = numLayers - 2; l >= 0; l--) {
			var oldDelta = delta;
			delta = hadamard(math.multiply(math.transpose(ann.layers[l+1].m_weights).valueOf(), oldDelta),
	...
	sink.Layer = function (numNeurons, numInputsPerNeuron, activationFn, activationFnPrime) {
		...
		//this.wNumRows = this.m_weights.valueOf().length;
	...
	sink.train = function (ann, trainingData, numEpochs) {
		...
		for(var ep = 0; ep < numEpochs; ep++) {
			...
			//	});
				var t = Date.now();
				console.log("single backprop time: " + (t - n));
				...
				for(var j = 0; j < ann.layers.length; j++) {
					var m_delta_rw = multMatrixScalar(ann.layers[j].m_delta_w, sink.conf.rate);
					var m_delta_rb = multMatrixScalar(ann.layers[j].m_delta_b, sink.conf.rate);

					ann.layers[j].m_weights = subtractMatrixMatrix(ann.layers[j].m_weights, m_delta_rw);
					ann.layers[j].m_biases = subtractMatrixMatrix(ann.layers[j].m_biases, m_delta_rb);
				}
